...
function setup() {
  ...
  dog = createSprite(width/2,height/2 + 100,10,10);
  ...
  feed.position(650,55);
  feed.mousePressed(feedDog);

  addFood = createButton("Add food");
  addFood.position(750,55);
  addFood.mousePressed(addFoods);

  input = createInput("Pet name");
  input.position(660,80);

  button = createButton("Confirm");
  button.position(710,105);
...
function draw() { 
  ...
  fill("red");
  textSize(20);
  text("Last fed: "+lastFed+":00",50,40);
}
